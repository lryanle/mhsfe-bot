...
function cmdRollDice(request){
  ...
  if (regex.test(request.text)) {
    ...
    if (val[1] < 1) {
      msg = "Hey I rolled no dice and the result is ... You're being a scuba kid";
    } else if (val[1] > 20) {
      msg = "If I can't fit more than 20 mallets in my non-existant hand";
    } else if (val[2] > 1000) {
      msg = "I'm automagically 3d printing dice with more than 1000 sides just for you. I'll get back to you in 37 years.";
    } else if (val[2] < 2) {
      msg = "Your results are: Drumline";
    } else {
      for (i = 0; i < val[1]; i++) {
